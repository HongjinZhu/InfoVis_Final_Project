...
export function HeatMap(props){
    ...
    const xScale = Scales.band(allCauses, 0, width);
    ...
    function cell(){
        ...
        years.map( d => {
            ...
            xScale={xScale} yScale={yScale} color={getColor(d,allCauses[i])} cause = {allCauses[i]} year = {d} />);
            }
        })
    ...
    return <svg width={WIDTH} height={HEIGHT}>
        ...
        {cell()}
        {allCauses.map(s => {
                        return <g key={s} transform={`translate(${xScale(s)+16},0)rotate(-30)`}>
                        <text style={{textAnchor:'start',fontSize:'12px'}}>{s}</text>
                        </g>
                    })}
        {years.map(m => {
                    return <text key={m} style={{textAnchor:'middle', fontSize:'9px'}} x={-20} y={yScale(m)+10}>{m}</text>
                })}
